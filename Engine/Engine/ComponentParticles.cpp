...
void ComponentParticle::BlitComponentInspector()
{
	...
	// Sprites
	ImGui::Text("Sprites");
	uint size = particle_textures.size();
	if (size == 0)ImGui::Text("NULL_MATERIAL");
	else
	{
		for (uint k = 0; k < size; k++)
		{
			particle_textures[k]->BlitUI();
		}
	}
...
void ComponentParticle::AddParticleTexture(ResourceMaterial* tex, bool ad_ref)
{
	particle_textures.push_back((ResourceMaterial*)tex);
...
bool ComponentParticle::Save(Serializer & array_root) const
{
	...
	for (uint k = 0; k < size; k++)
	{
		...
		textures_array.InsertArrayInt(particle_textures[k]->GetID());
	}
...
bool ComponentParticle::Load(Serializer & data, std::vector<std::pair<Component*, uint>>& links)
{
	...
	for (uint k = 0; k < size; k++)
	{
		...
		if (mat != nullptr)AddParticleTexture(mat);
	}
