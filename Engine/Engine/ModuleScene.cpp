...
void ModuleScene::BlitComponentsWindow(GameObject* target)
{
	if (target != nullptr)
	{
		...
		{
			bool selected = ImGui::Selectable("Particle Emitter");

			if (selected)
			{
				App->audio->PlayFxForInput(FX_ID::CHECKBOX_FX);

				//Generate a component particle
...
void ModuleScene::PlayGame()
{
	if (scene_update_state == PLAY_SCENE_STATE || scene_update_state == PAUSE_SCENE_STATE)
	{
		...
		App->textures->sparks_id = App->textures->sparks_texture;
		App->textures->explode_id = App->textures->explode_texture;
